...
let make = async function(port, type, preset = 0) {
    ...
    let jarx = spawn("cd " + box + " && sudo -S java -Xmx1G -jar " + type + " --online-mode false --max-players 50 --port " + port + " --nogui", {shell: true});
    ...
    await new Promise(async(resolve, reject) => {
        ...
        jarx.on('exit', function (code) {
            let msg = "[ZSYS] Exited with code " + code + "!";
            obj[iid].lines.push(msg);
            fs.appendFileSync(path.join(box, "z.log"), "\n" + msg);
        });
    }).then(async() => {
